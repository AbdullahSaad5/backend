...
export const realTimeEmailSyncCron = () => {
      ...
      for (let i = 0; i < accountsNeedingSetup.length; i++) {
        ...
        try {
          ...
          if (isRetry) {
            ...
            logger.info(`📧 [${i + 1}/${accountsNeedingSetup.length}] Setting up webhook for: ${account.emailAddress}`);
          }
  ...
  logger.info("📅 Real-time email sync cron jobs scheduled");
};
